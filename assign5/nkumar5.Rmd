...
**source code for package would be updated as a link here soon:**
...
Date: 2015-08-02
Author: Neeraj Kumar
Maintainer: Neeraj Kumar <nekumar@mix.wvu.edu>
...
```{r}
### => R CMD INSTALL --no-multiarch --with-keep.source oneway
```
...
library(utils)
library(tools)
#install_github("neerajnj10/oneway") #working on it currently to commit & push files.
library(oneway)
...
print.help("lsmeans")
#print.help("coagulation") #doesn't read the help for coagulation data.
```
The help files should provide all relevant information.

#### 9. Illustrate your functions using:  
```{r oneway.example}
#example(oneway) ##only summary.oneway is not working
```
